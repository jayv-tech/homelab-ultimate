...
installDock()
{
  clear
    ...
    #Updates check
        checkForUpdates() 
        {
            #Running pacman update to check for the updates available.
                echo ""
                echo -n " Hey $username, I am now running the package updates. This is necessary to proceed with installing Docker."
                ($SUDO pacman -Syy) >> ~/docker/homelab-install-script.log & spinner $!
                echo " It's Done."
        ...
        DockCheck()
        {
            ...
            echo ""
                ...
                (sudo pacman --noconfirm -S docker) >> ~/docker/homelab-install-script.log 2>&1 &
                    spinner $!
                echo ""
                ...
                (sudo systemctl start docker.service) >> ~/docker/homelab-install-script.log 2>&1
                sleep 10s
                echo ""
                echo " Done."
                sleep 1s
            ...
            if [[ "$ISCOMP" == *"command not found"* ]]; then
                ...
                (sudo pacman -Sy) >> ~/docker/homelab-install-script.log 2>&1
                (sudo pacman --noconfirm -S docker-compose) >> ~/docker/homelab-install-script.log 2>&1 
                ...
                (sudo systemctl enable docker.service) >> ~/docker/homelab-install-script.log 2>&1
...
($SUDO pacman --noconfirm -S wget curl git libnewt) > ~/docker/homelab-install-script.log & spinner $!
