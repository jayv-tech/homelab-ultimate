...
installDock()
{
  clear
    ...
    #Updates check
        checkForUpdates() 
        {
            #Running dnf update to check for the updates available.
                echo ""
                echo -n " Hey $username, I am now running the package updates. This is necessary to proceed with installing Docker."
                ($SUDO dnf check-update) >> ~/docker/homelab-install-script.log & spinner $!
                echo " It's Done."
...
(sudo dnf install -y bind-utils newt) > ~/docker/homelab-install-script.log
